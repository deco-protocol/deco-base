...
contract ZeroAdapterERC20 is DSMath {
    ...
    function exit(address src, address dst, bytes32 class_, uint zbal_) external approved(src) {
        require(address(tokens[class_]) != address(0), "zero/token-not-deployed");

        core.moveZero(src, address(this), class_, mulu(zbal_, RAY)); // to rad
        ERC20(tokens[class_]).mint(dst, zbal_);
    }

    function join(address src, address dst, bytes32 class_, uint zbal_) external approved(src) {
        require(address(tokens[class_]) != address(0), "zero/token-not-deployed");
...
contract ClaimAdapterERC20 is DSMath {
    ...
    function deployToken(uint issuance, uint activation, uint maturity) public returns (address) {
        bytes32 class_ = keccak256(abi.encodePacked(issuance, activation, maturity));
        require(address(tokens[class_]) == address(0), "futureclaim/token-exists");
    ...
    function exit(address src, address dst, bytes32 class_, uint cbal_) external approved(src) {
        require(address(tokens[class_]) != address(0), "claim/token-not-deployed");

        core.moveClaim(src, address(this), class_, cbal_);
        ERC20(tokens[class_]).mint(dst, cbal_);
    }

    function join(address src, address dst, bytes32 class_, uint cbal_) external approved(src) {
        require(address(tokens[class_]) != address(0), "claim/token-not-deployed");

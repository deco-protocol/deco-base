...
contract Core is DSMath {
    ...
    event MintZero(address indexed usr, bytes32 indexed class_, address yield, uint maturity, uint zbal);
    event BurnZero(address indexed usr, bytes32 indexed class_, address yield, uint maturity, uint zbal);
    event MintClaim(address indexed usr, bytes32 indexed class_, address yield, uint issuance, uint maturity, uint cbal);
    event BurnClaim(address indexed usr, bytes32 indexed class_, address yield, uint issuance, uint maturity, uint cbal);
    event MintFutureClaim(address indexed usr, bytes32 indexed class_, address yield, uint issuance, uint activation, uint maturity, uint cbal);
    event BurnFutureClaim(address indexed usr, bytes32 indexed class_, address yield, uint issuance, uint activation, uint maturity, uint cbal);
    event MoveZero(address indexed src, address indexed dst, bytes32 indexed class_, uint zbal);
    event MoveClaim(address indexed src, address indexed dst, bytes32 indexed class_, uint cbal);
    ...
    function mintZero(address yield, address usr, uint maturity, uint zbal_) public onlyThisOrYieldAdapter(yield) {
        bytes32 class_ = keccak256(abi.encodePacked(yield, maturity)); // calculate zero class with adapter address and maturity timestamp

        zBal[usr][class_] = addu(zBal[usr][class_], zbal_);
        totalSupply[class_] = addu(totalSupply[class_], zbal_);
        emit MintZero(usr, class_, yield, maturity, zbal_);
    }

    function burnZero(address yield, address usr, uint maturity, uint zbal_) public onlyThisOrYieldAdapter(yield) {
        bytes32 class_ = keccak256(abi.encodePacked(yield, maturity));
        ...
        emit BurnZero(usr, class_, yield, maturity, zbal_);
    }

    function mintClaim(address yield, address usr, uint issuance, uint maturity, uint cbal_) public onlyThisOrYieldAdapter(yield) {
        bytes32 class_ = keccak256(abi.encodePacked(yield, issuance, maturity));

        cBal[usr][class_] = addu(cBal[usr][class_], cbal_);
        emit MintClaim(usr, class_, yield, issuance, maturity, cbal_);
    }
    ...
    function burnClaim(address yield, address usr, uint issuance, uint maturity, uint cbal_) public onlyThisOrYieldAdapter(yield) {
        ...
        emit BurnClaim(usr, class_, yield, issuance, maturity, cbal_);
    }
    ...
    function mintFutureClaim(address yield, address usr, uint issuance, uint activation, uint maturity, uint cbal_) public onlyThisOrYieldAdapter(yield) {
        bytes32 class_ = keccak256(abi.encodePacked(yield, issuance, activation, maturity));

        cBal[usr][class_] = addu(cBal[usr][class_], cbal_);
        emit MintFutureClaim(usr, class_, yield, issuance, activation, maturity, cbal_);
    }
    ...
    function burnFutureClaim(address yield, address usr, uint issuance, uint activation, uint maturity, uint cbal_) public onlyThisOrYieldAdapter(yield) {
        ...
        cBal[usr][class_] = subu(cBal[usr][class_], cbal_);
        emit BurnFutureClaim(usr, class_, yield, issuance, activation, maturity, cbal_);
    }
    ...
    // --- Transfer Functions ---
    function moveZero(address src, address dst, bytes32 class_, uint zbal_) external approved(src) {
    ...
    function moveClaim(address src, address dst, bytes32 class_, uint cbal_) external approved(src) {
    ...
    function snapshot(address yield) external {
        ...
        amp[yield][block.timestamp] = amp_;

        latestAmpTimestamp[yield] = block.timestamp; // update latest amp timestamp

        emit NewAmp(yield, block.timestamp, amp_);
    }
